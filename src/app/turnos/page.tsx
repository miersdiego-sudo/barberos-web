...
import { getTurnos, getPromos, crearTurno } from '@/lib/supabaseClient'

const barberos = ['Marcos', 'Juanjo', 'Lucas']
...
export default function TurnosPage() {
  ...
  const [servicio, setServicio] = useState<typeof servicios[number] | null>(null)
  ...
  const promoActiva = fecha
    ? promos.find(p => fecha >= p.inicio && fecha <= p.fin)
    : null
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <div style={{ background: '#2B2B2B', borderRadius: 8, padding: 20, textAlign: 'left', marginBottom: 24 }}>
            ...
            <p><strong style={{ color: '#C8862B' }}>Cliente:</strong> {nombre} ({telefono})</p>
          </div>
          ...
          <>
            ...
            {barberos.map(b => (
              <button key={b} onClick={() => { setBarbero(b); setPaso(2) }}
                style={barbero === b ? btnSel : btnBase}>
                {b}
              </button>
            ))}
          </>
              ...
              <>
                ...
                {horario && (
                  <div style={{ marginTop: 20, padding: 12, background: '#2B2B2B', borderRadius: 6, border: '1px solid #3a3a3a' }}>
                    ...
                    <p style={{ fontSize: 14, color: '#ccc' }}>
                      {horario} → {aHora(aMinutos(horario) + servicio!.duracion)}
                    </p>
                    <p style={{ fontSize: 16, color: '#F2EFE9', marginTop: 4 }}>
                      <strong>Total:</strong> {formatearPrecio(precioFinal)}
                      {promoActiva && <span style={{ color: '#D9A441', fontSize: 13, 
  marginLeft: 8 }}>({promoActiva.porcentaje}% OFF)</span>}
                    </p>
                  </div>
                )}
                {fecha && horario && (
        ...
        {paso === 4 && (
          <>
            ...
            <button onClick={confirmar} disabled={!nombre || !telefono}
              style={{
                padding: '12px 24px', background: '#C8862B', color: '#1A1A1A', border: 'none',
                borderRadius: 6, cursor: 'pointer', fontWeight: 700, fontSize: 16, width: '100%',
                opacity: (!nombre || !telefono) ? 0.5 : 1
              }}>
              Confirmar turno
            </button>
